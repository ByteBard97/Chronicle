...
def test_derives_trivial_state_from_appended_events():
    log = EventLog()
    log.append(NPCDied(tick=10, save_uuid="s1", generation=0, npc_id="jarl_balgruuf", cause="assassination", killer_id="unknown"))
    log.append(CrimeWitnessed(tick=10, save_uuid="s1", generation=0, witness_id="proventus", perpetrator_id="unknown", crime_type="murder"))
    log.append(RumorHeard(tick=12, save_uuid="s1", generation=0, hearer_id="hulda", source_id="proventus", rumor_id="r1", content="The Jarl is dead."))
...
def test_forking_a_branch_excludes_the_abandoned_suffix():
    log = EventLog()
    log.append(NPCDied(tick=10, save_uuid="s1", generation=0, npc_id="jarl_balgruuf", cause="assassination"))
    log.append(RumorHeard(tick=12, save_uuid="s1", generation=0, hearer_id="hulda", source_id="proventus", rumor_id="r1", content="The Jarl is dead."))
    # Player reloads a save from before this point -- the third event was never seen in-game.
    log.append(RumorHeard(tick=14, save_uuid="s1", generation=0, hearer_id="ysolda", source_id="hulda", rumor_id="r1", content="The Jarl was murdered by the Thalmor."))
    ...
    log.append(NPCDied(tick=20, save_uuid="s1", generation=1, npc_id="jarl_balgruuf", cause="illness"))
