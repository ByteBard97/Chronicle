...
| 29 — Tier-3-rich demo run | **in progress, worker (Kimi) out of session usage mid-lane, ~2026-08-23 evening** — `scenarios/run_tier3_demo.py` written (238 lines, uncommitted) but no `runs/tier3-demo-01/` yet exists and no delivery report was filed | **noted by Claude while investigating project state during Kimi's outage**: running the script as left on disk raises `ValueError: a retelling cannot precede the teller's last rehearsal of it` from `chronicle/driver.py:933`'s automatic encounter-propagation path (`claims.py:348`'s guard) — a scenario-authoring tick-ordering issue in the script's scripted actions vs. the schedule's automatic encounters, not an engine bug (no engine file is touched by this lane). Left as-is, uncommitted, for Kimi to resume — not fixed, since this is Track A and outside any dispatch to another worker |
