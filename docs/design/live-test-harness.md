...
| 20 | death | `0002C90F.kill` (Brenuin, expendable, dot-syntax targets the ref directly -- no `prid` selection step, see below); afterwards `0002C90F.resurrect` | `events.jsonl` gains `npc_died` with `npc_id=brenuin`, `gamets>0` within 15s; listener log shows the POST. |
