...
| 3 | Avoidance pair: "Ysolda and Carlotta Valentia" | **Would silently fail for the wrong reason.** Only 4 of 171 possible pairs have real (non-placeholder) FormIDs wired into `AvoidanceGlobals.cpp`'s illustrative C++ lookup table: `(nazeem, ysolda)`, `(carlotta_valentia, saffir)`, `(amren, brenuin)`, `(fralia_gray_mane, idolaf_battle_born)`. Ysolda+Carlotta is not one of them — `ResolveAvoidancePairGlobal` would return `std::nullopt` for that pair every time, and `AvoidancePoller.cpp` would log `"no ChronicleAvoidingPair_* global authored yet"` forever, which looks identical to "avoidance doesn't work" if you don't know to check the log line. Recommended pair: **`nazeem`/`ysolda`** (see reasoning below). | `AvoidanceGlobals.cpp` |
...
- **Use `nazeem`/`ysolda`, not "Ysolda and Carlotta Valentia"**
  (correction #3). Of the 4 pairs with real FormIDs wired into
  `AvoidanceGlobals.cpp` — `(nazeem, ysolda)`, `(carlotta_valentia,
  saffir)`, `(amren, brenuin)`, `(fralia_gray_mane,
  idolaf_battle_born)` — nazeem/ysolda were the closest together in the
