...
| `mutation_applied` | 2 | `claim_id`, `parent_variant_id`, `variant_id`, `slot` (string), `old_value`, `new_value`, `mutation_id` (string — the seeded mutation id the variant tree labels edges with), `roll_key` (purpose `mutation.slot` / `mutation.value` \| null — null on the value side when the substitution was deterministic, e.g. T2.4's allegiance-mapped value; the §4:121 idiom) |
