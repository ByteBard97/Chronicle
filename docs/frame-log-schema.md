...
| `crime_witnessed` | 0 | `witness_id` (string), `perpetrator_id` (string), `crime_type` (string), `victim_id` (string \| null — the crime's own victim, when there is a specific one; `None` means no specific human victim or unknown, and always routes to the bystander/no-grudge cascade), `location_id` (string \| null) |
...
| `status_changed` | 5 — **reserved** | `npc_id` (string), `status_kind` (string — e.g. `"thane"`, `"role_appointed"`), `detail` (string — e.g. hold/role name), `location_id` (string \| null). An honest anchor for status/roster changes (supersedes the lane-23/26 `RumorHeard` anchoring precedent for new fixtures). Fields defined 2026-08-23 (lane-26 backlog, coordinator) |
| `role_installed` | 5 — **reserved** | `role_id` (string), `title` (string), `institution_id` (string), `duties` (list of `{name, lapse_status_kind}`), `holder_id` (string \| null). The role-roster anchor: emitted once per role at installation (run start or later), so readers can reconstruct the roster without the fixture; vacancy derives from `npc_died` replay (lane-47 design), lapse/succession from `status_changed`. Fields defined 2026-08-24 (coordinator, ahead of lane 51) |
| `role_lapse` | 5 — **superseded, will not be filled** | duty-lapse effects travel as `status_changed` events instead (`status_kind = "duty_lapsed"`, detail = the duty name; succession uses `"role_appointed"`) — retired 2026-08-23 (lane-44 design doc S4, coordinator ruling O3) |
...
| `transmitted` | 1 | `claim_id`, `teller_id`, `teller_belief_id`, `hearer_id`, `hearer_belief_id`, `evidence_id`, `variant` (`{variant_id, parent_variant_id, slots, mutated_slot: string \| null}`), `location_id`, `trust_applied` (number, [0,1] \| null — amended 2026-08-23, rule 20, `docs/design/trust-discounted-retelling.md`: the trust value `claims.retell()` applied to this transmission's confidence, or null when the rule was disabled or the caller never supplied one; on a re-hearing (below) this is the value the driver looked up, but a re-hearing mints nothing, so no confidence was actually recomputed from it). A variant is created on every transmission **to an uninformed hearer** (mutated or not) per `claims.retell()`. A same-content re-tell to an already-informed hearer is a **re-hearing** (rule 7 exposure/distinct-source counting): the record references the *existing* variant and hearer-belief ids and nothing is minted (amended 2026-08-23, lane-12 overseer review, Conflict 2) |
...
| `mutation_applied` | 2 | `claim_id`, `parent_variant_id`, `variant_id`, `slot` (string), `old_value`, `new_value`, `mutation_id` (string — the seeded mutation id the variant tree labels edges with), `roll_key` (purpose `mutation.slot` — always the *slot* roll's key, including when the value substitution was deterministic (T2.4's allegiance path): the slot is still rolled; the value roll's key is derivable from it, never stored, and a deterministic value simply has no value roll to derive) |
| `supersession` | 2 | `holder_id`, `claim_id`, `loser_variant_id` (string \| null), `winner_variant_id` (string \| null), `resolution_rule` (string — the named resolution rule), `confidence_dent` (number, [0,1]), `teller_id`, `teller_belief_id`, `evidence_id`, `winner_belief_id`, `trust_applied` (number, [0,1] \| null — amended 2026-08-23, rule 20, `docs/design/trust-discounted-retelling.md`: the trust value applied to a `challenger_wins` adoption's confidence; null for the challenge-repelled branch, which never uses `RETELL_CONFIDENCE_DECAY`, or when the rule was disabled/no trust was supplied). A separate record pointing at both variants — the loser is never mutated in place. A null variant id names the claim's original telling (witness-held, un-varianted — the same idiom as `transmitted`'s `mutated_slot` above). The final four fields (amended 2026-08-23, lane-12 pre-implementation review, findings 1–2) make resolution re-executable from the trace alone: the contested hearing is a real Evidence record appended to the winner's belief, and post-keyframe replay must be able to rebuild it |
